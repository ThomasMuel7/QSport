...
def compute_fatigue_dataframe(
    ...
    Paramètres:
    - api_key: clé BSD,
    - matches_df: DataFrame source à enrichir (mode DataFrame),
    - event_id_column: nom de la colonne contenant l'id BSD du match (par défaut BSD_EVENT_ID),
    - tz: timezone BSD pour les dates,
    - output_csv_path: chemin de sortie CSV (facultatif).

    Retour:
    - DataFrame pandas avec toutes les métriques.

    passer matches_df avec la colonne BSD_EVENT_ID (ou event_id_column).
       Les colonnes de métriques sont ajoutées directement au DataFrame fourni.
    """
    if matches_df is None:
        raise ValueError("Tu dois fournir matches_df.")

    client = BSDClient(api_key=api_key)
    geo_service = GeoService()

    def _compute_row(event_id_value: Any) -> Dict[str, Any]:
        if pd.isna(event_id_value):
            return {
                "event_id": None,
                "error": f"Missing {event_id_column}",
            }

        try:
            event_id_int = int(event_id_value)
            return compute_fatigue_for_match(
                client=client,
            ...
            return {"event_id": int(event_id_value), "error": f"HTTPError: {exc}"}
        except Exception as exc:  # pylint: disable=broad-except
            return {"event_id": int(event_id_value), "error": f"Error: {exc}"}
    
    # Mode DataFrame: enrichissement en place de matches_df.
    if event_id_column not in matches_df.columns:
        raise ValueError(f"La colonne {event_id_column} est absente du DataFrame.")

    results = [_compute_row(event_id_value) for event_id_value in matches_df[event_id_column].tolist()]
    metrics_df = pd.DataFrame(results)

    for col in metrics_df.columns:
        matches_df[col] = metrics_df[col].to_numpy()
...
if __name__ == "__main__":
    ...
    # 3) Enrichit directement le DataFrame source à partir de la colonne BSD_EVENT_ID.
    enriched_df = compute_fatigue_dataframe(
        api_key=API_KEY,
        matches_df=input_matches_df,
        event_id_column="BSD_EVENT_ID",
        tz="UTC",
        output_csv_path="forme.csv",
    )
    print(enriched_df)
